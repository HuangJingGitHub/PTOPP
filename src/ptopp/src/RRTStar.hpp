...
bool RRTStarPlanner::Plan(Mat source_img, int max_graph_size, bool plan_in_interior, float interior_diameter) {   
    ...
    while (GRAPH_SIZE < MAX_GRAPH_SIZE) {
        ...
        if (cost_function_type_ == 0 && plan_in_interior 
            && MinDistanceToObstaclesVec(obstacles_, new_node->pos) < interior_diameter) {
            delete new_node;
            continue;
        }
...
float RRTStarPlanner::NewCost(PathNode* near_node, PathNode* new_node) {
    ...
    if (check_all_passages_) {
        passed_width_vec = GetPassageWidthsPassed(passage_pts_, near_node->pos, new_node->pos);
    }
    else {
...
void RRTStarPlanner::UpdateSubtree(PathNode* new_parent, PathNode* child) {
    ...
    if (cost_function_type_ > 1) {
        if (check_all_passages_) {
            passed_width_vec = GetPassageWidthsPassed(passage_pts_, new_parent->pos, child->pos);
        }
        else {
