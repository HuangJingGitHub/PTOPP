...
void SVITest(const int obs_num = 50, const int side_len = 30, 
    ...
    for (int i = 0; i < DG_check_res.pairs.size(); i++) {
        vector<int> pair = DG_check_res.pairs[i];
        ...
        Scalar orange = Scalar(0, 0.65, 1),
               light_blue = Scalar(0.8824, 0.4118, 0.2549);
        DrawDashedLine(back_img, SVI_key_pts[0][0], SVI_key_pts[0][1], light_blue, 2, 10);
        DrawDashedLine(back_img, SVI_key_pts[1][0], SVI_key_pts[1][1], light_blue, 2, 10);
        DrawDashedLine(back_img, SVI_key_pts[2][0], SVI_key_pts[2][1], Scalar(0, 0, 255), 2, 10);
    }
    
    string save_directory = "./src/ptopp/src/img/0226/";
...
void EVDGConsistencyTest(const int test_num = 100, const int obs_num = 50, const int side_len = 30, 
    ...
    for (int test_idx = 0; test_idx < test_num; test_idx++) {
        vector<PolygonObstacle> obs_vec = GenerateRandomObstacles(obs_num, back_img.size(), side_len, true);
        ...
        if (EV_check_res.pairs.size() != DG_check_res.pairs.size()) {
            inconsistency_num++;
            cout << "Passage number in extended visibility check vs passage number in Delaunay Graph check:\n"
                 << EV_check_res.pairs.size() << " vs " << DG_check_res.pairs.size() << "\n";
            set<vector<int>> EV_psg_set(EV_check_res.pairs.begin(), EV_check_res.pairs.end()),
                             DG_psg_set(DG_check_res.pairs.begin(), DG_check_res.pairs.end());
            ...
            for (auto psg : DG_exclusive) {
                cout << psg[0] << "-" << psg[1] << "\n";
                for (int i = 0; i < DG_check_res.pairs.size(); i++)
                    if (DG_check_res.pairs[i][0] == psg[0] && DG_check_res.pairs[i][1] == psg[1]) {
                        cout << DG_check_res.pts[i][0] << ", " << DG_check_res.pts[i][1] << "\n";
                        DrawDashedLine(back_img, DG_check_res.pts[i][0], DG_check_res.pts[i][1], Scalar(0, 0, 255), 2);
                        circle(back_img, (DG_check_res.pts[i][0] + DG_check_res.pts[i][1]) / 2, 
                                cv::norm(DG_check_res.pts[i][0] - DG_check_res.pts[i][1]) / 2, Scalar(0, 0, 255), 2);
                        break;
                    }
            }
            ...
            for (int i = 0; i < DG_check_res.pts.size(); i++)
                DrawDashedLine(back_img, DG_check_res.pts[i][0], DG_check_res.pts[i][1], Scalar(0, 0, 0), 2);

            vector<vector<int>> DG_adj = DelaunayTriangulationObstables(obs_vec);
            for (int i = 0; i < DG_adj.size(); i++) 
                for (int j : DG_adj[i]) 
                    if (i < j)
                        DrawDashedLine(DG_img, obs_centroids[i], obs_centroids[j], Scalar(0, 0, 255), 2);
...
void GabrielCellTest(const int test_num = 100, const int obs_num = 50, const int side_len = 30, 
    ...
    Passages DG_check_res = PassageCheckDelaunayGraphWithWalls(obs_vec);        
    vector<vector<int>> cells = ReportGabrielCells(obs_vec, DG_check_res.pairs, false);
    vector<PolygonCell> cell_info = GetGabrielCellsInfo(cells, DG_check_res);
...
int main(int argc, char** argv) {
    ...
    // EVDGConsistencyTest(10, 100, 30, back_img);
    SVITest(20, 40, back_img, false);
