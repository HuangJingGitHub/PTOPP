...
#include <vector>
#include <queue>
#include "../3d/kd_tree_3d.hpp"

using namespace std;
using namespace cv;
using namespace std::chrono;

PathNode* FindNearestNodeByBFS(PathNode* root, Point3f test_pt) {
    queue<PathNode*> level;
    level.push(root);
    PathNode* res;
...
int main() {
    srand(time(NULL));
    float x_range = 100, y_range = 100, z_range = 100,
          div_x = RAND_MAX / x_range,
          div_y = RAND_MAX / y_range,
          div_z = RAND_MAX / z_range;
    float x_min = 40, x_max = 60, y_min = 40, y_max = 60, z_min = 40, z_max = 60;
    int node_num = 1000, range_search_cnt = 0;

    vector<PathNode*> node_ptr_vec(node_num);
    for (int i = 0; i < node_num; i++) {
        float x = rand() / div_x, y = rand() / div_y, z = rand() / div_z;
        node_ptr_vec[i] = new PathNode(Point3f(x, y, z));
    ...
    int test_pt_num = 100;
    vector<Point3f> test_pt_vec(test_pt_num);
    vector<PathNode*> correct_res_vec(test_pt_num), test_res_vec(test_pt_num);
    for (int i = 0; i < test_pt_num; i++) {
        float x = rand() / div_x, y = rand() / div_y, z = rand() / div_z;
        test_pt_vec[i] = Point3f(x, y, z);
    }
    ...
    vector<PathNode*> range_search_res = test_kd_tree.RanageSearch(x_min, x_max, y_min, y_max, z_min, z_max);
    cout << "Range search for " << x_min << " <= x <= " << x_max << ", " << y_min << " <= y <= " << y_max << ", " << z_min << " <= z <= " << z_max << ":\n";
