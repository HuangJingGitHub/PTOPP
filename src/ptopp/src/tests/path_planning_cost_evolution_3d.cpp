...
void PlanningCostEvolutionTest3d(int test_num, int obs_num, int sample_num, vector<PolygonObstacle3d>& obs_vec) {
    Mat back_img(Size(1000, 600), CV_64FC3, Scalar(255, 255, 255));
    int obs_side_len = 60;
    float config_height = 205, interior_depth = 10, rrts_step = 100;
    Point3f start = Point3f(50, 50, 1), target = Point3f(back_img.size().width - 50, back_img.size().height - 50, 200);
    float rrt_step = 50;
    ...
    for (int test_idx = 0; test_idx < test_num; test_idx++) {
        ...
        RRTStarPlanner3d rrts_len(start, target, obs_vec, rrt_step, back_img.size(), config_height, 0),
                        ...
                        rrts_mpw(start, target, obs_vec, rrt_step, back_img.size(), config_height, 2);
...
int main(int argc, char** argv) {
    ...
    PlanningCostEvolutionTest3d(test_num, obs_num, sample_num, obs_vec);
    return 0;
}