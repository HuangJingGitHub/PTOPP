...
void PlanningTimeTest3d(int test_num, int obs_num, int sample_num, vector<PolygonObstacle3d>& obs_vec) {
    Mat back_img(Size(1000, 600), CV_64FC3, Scalar(255, 255, 255));
    int obs_side_len = 60;
    float config_height = 400, interior_depth = 10, rrts_step = 100;
    ...
    for (int test_idx = 0; test_idx < test_num; test_idx++) {
        cout << "\n\nsample number: " << sample_num << " test iteration: " << test_idx << "\n";
        ...
        PRMStarPlanner3d prms_len(start, target, obs_vec, back_img.size(), config_height, 0, false, false, 1, sample_num),
                        prms_interior(start, target, obs_vec, back_img.size(), config_height, 0, false, true, interior_depth, sample_num),
                        prms_clearance(start, target, obs_vec, back_img.size(), config_height, 1, false, false, 1, sample_num),
                        prms_gpw(start, target, obs_vec, back_img.size(), config_height, 3,false, false, 1, sample_num),
                        prms_gpw_all(start, target, obs_vec, back_img.size(), config_height, 3, true, false, 1, sample_num);
        ...
        bool planned_interior = rrts_interior.Plan(sample_num, true, interior_depth);
        ...
        start_time = high_resolution_clock::now();
        bool planned_gpw = rrts_gpw.Plan(sample_num);
        end_time = high_resolution_clock::now();
        duration_time = duration_cast<milliseconds>(end_time - start_time);   
        rrts_gpw_time[test_idx] = (float)duration_time.count();

        start_time = high_resolution_clock::now();
        bool planned_gpw_all = rrts_gpw_all.Plan(sample_num);
        end_time = high_resolution_clock::now();
        duration_time = duration_cast<milliseconds>(end_time - start_time); 
    ...
    file_name = "planning_time_3d_obs_" + to_string(obs_num) + "_test_" + to_string(test_num) + "_sample_" + to_string(sample_num / 1000) + "k_";
...
int main(int argc, char** argv) {
    ...
    for (int sample_num = 1000; sample_num <= 20000; sample_num += 1000)
        PlanningTimeTest3d(test_num, obs_num, sample_num, obs_vec);
    return 0;
}