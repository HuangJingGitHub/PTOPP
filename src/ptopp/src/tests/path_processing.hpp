...
void DrawPath(Mat img, const vector<PathNode*>& path, 
            Scalar color = Scalar(255, 0, 0), 
            int thickness = 2,
            Point2f shift = Point2f(0, 0)) {
    for (int i = 0; i < (int)path.size() - 1; i++)
...
void DrawDashedPath(Mat img, const vector<Point2f>& path, 
                    Scalar color = Scalar(255, 0, 0), 
                    int thickness = 2,
                    Point2f shift = Point2f(0, 0), 
                    float dash_len = 2) {
    for (int i = 0; i < (int)path.size() - 1; i++)
        DrawDashedLine(img, path[i] + shift, path[i + 1] + shift, color, thickness, dash_len);
}

void DrawDashedPath(Mat img, const vector<PathNode*>& path, 
                    Scalar color = Scalar(255, 0, 0), 
                    int thickness = 2,
                    Point2f shift = Point2f(0, 0),
                    float dash_len = 10) {
    for (int i = 0; i < (int)path.size() - 1; i++)
        DrawDashedLine(img, path[i]->pos + shift, path[i + 1]->pos + shift, color, thickness, dash_len);
}
...
vector<Point2f> QuadraticBSplineSmoothing(const vector<PathNode*>& node_path) {
    vector<Point2f> res;
    if (node_path.size() < 3) {
...
vector<vector<Point2f>> GeneratePathSet(const vector<Point2f>& initial_feedback_pts, 
    ...
    vector<PathNode*> pivot_path = planner.GetPath();
    vector<PathNode*> sparse_pivot_path;
...
/*         Upgraded         */
/****************************/
pair<vector<int>, vector<int>> RetrievePassedPassages(const vector<PathNode*>& raw_pivot_path, 
                                                      const vector<vector<Point2f>>& passage_pts) {
    vector<int> res_passage_indices, res_path_intersection_indices;
    for (int path_node_idx = 0; path_node_idx < (int)raw_pivot_path.size() - 1; path_node_idx++) {
    ...
    for (int path_idx = 0; path_idx < raw_pivot_path.size() - 1; path_idx++) {
        for (int passage_idx = 0; passage_idx < passage_pts.size(); passage_idx++) {
            if (SegmentIntersection(raw_pivot_path[path_idx], raw_pivot_path[path_idx + 1], 
                ...
                if (res_passage_indices.size() > 0 && passage_idx == res_passage_indices.back()){
                    cout << "Repeated passage traversal detected\n";
                    cout << "Passage info: " << passage_idx << ", " << passage_pts[passage_idx][0] << "-" << passage_pts[passage_idx][1] << "\n";
                    for (int i = path_idx - 1; i <= path_idx + 1; i++)
                        cout << raw_pivot_path[i] << ", ";
                }
