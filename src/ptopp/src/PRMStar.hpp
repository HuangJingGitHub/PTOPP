...
void PRMStarPlanner::QueryPath(Mat source_img) {
    ...
    for (int i = 0; i < node_vec_.size(); i++) {
        ...
        if (min_cost_heap.empty()) {
            // An unconnected graph is constructed.
            plan_success_ = false;
            return;
        }
...
vector<float> PRMStarPlanner::QueryCostLog(vector<int> query_samples) {
    ...
    for (int query_idx = 0; query_idx < query_samples.size(); query_idx++) {
        ...
        for (int i = 0; i < query_sample_num; i++) {
            ...
            if (cv::norm(node->pos - target_node_->pos) <= radius_alg && EdgeObstacleFree(node, target_node_)) {
                best_target_cost = min(best_target_cost, NewCost(node, target_node_));
            }
        ...
        cost_log[query_idx] = best_target_cost;
        ...
        for (int j = 0; j < query_sample_num; j++) {
            ...
            node->parent = nullptr;
            node->min_passage_width = 10000;
            node->cur_passage_widths.clear();
            node->sorted_passage_list.clear();
        }
        start_node_->len = 0;
        cout << query_sample_num << " with min cost " << best_target_cost << "\n";
    }  
    return cost_log;  
}
...
PathNode* PRMStarPlanner::FindParentByCost(vector<PathNode*> node_vec, PathNode* new_node) {
    ...
    for (PathNode* node : node_vec) {
        ...
        if (cost_function_type_ == 1)
            ...
            // cost = -SegMinDistanceToObstaclesVec(obstacles_, node->pos, new_node->pos);
...
float PRMStarPlanner::NewCost(PathNode* near_node, PathNode* new_node) {
    ...
    if (cost_function_type_ == 1)
        ...
        return -min(-near_node->cost, SegMinDistanceToObstaclesVec(obstacles_, near_node->pos, new_node->pos));
...
void PRMStarPlanner::UpdateSubtree(PathNode* new_parent, PathNode* child) {
    ...
    if (cost_function_type_ == 1)
        child->clearance = SegMinDistanceToObstaclesVec(obstacles_, new_parent->pos, child->pos);
